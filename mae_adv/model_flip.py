...
class MAE_ViT(torch.nn.Module):
    ...
    def shuffle(self, patches, ratio=None, forward_indexes=None, backward_indexes=None, ):
        T, B, C = patches.shape  # length, batch, dim

        remain_T = int(T * (1 - ratio))
        if forward_indexes is None and backward_indexes is None:
            noise = torch.rand(T, B, device=patches.device)
            forward_indexes = torch.argsort(noise, dim=0)
            backward_indexes = torch.argsort(forward_indexes, dim=0)
    ...
    def forward_clip(self, feats, feats_adv):
        ...
        return logits_per_ori, logits_per_adv

    def forward_feat(self, img, ratio=0.75):
        img = Normalize(CIFAR10_MEAN, CIFAR10_STD, preprocess=False)(img)
        patches = self.encoder.patchify(img)
        patches = rearrange(patches, 'b c h w -> (h w) b c')
        patches = patches + self.encoder.pos_embedding

        patches, forward_indexes, backward_indexes = self.shuffle(patches, ratio)
