            ...
            with tqdm(total=train_step, desc=f'Train Epoch {e + 1}/{self.total_epoch}', postfix=dict,
                ...
                for img, label in iter(self.train_dataloader):
                    ...
                    with self.accelerator.autocast():
                        ...
                        adv_img = adaad_inner_loss(self.model, self.teacher_model, img)

                        # self.model.train()

                        # logits, logits_distill = self.model(adv_img)
                        # logits_org = self.model(img)
                        logits, logits_distill = self.model.forward_train(adv_img)
                        #
                        # adv_img = apgd.perturb(img, label, x_init=adv_img)
                        # logits, logits_distill = self.model.forward_train(adv_img)
                        ...
                        # loss = distill_loss(logits,
                        #                     teacher_logits)  #+ self.loss_fn(logits,label)  #+distill_loss(logits_org,teacher_logits_org)  #+self.loss_fn(self.model(img), label)
                        kl_loss1 = nn.KLDivLoss()(F.log_softmax(logits_distill, dim=1),
                                                  F.softmax(teacher_logits.detach(), dim=1))
                        kl_loss2 = nn.KLDivLoss()(F.log_softmax(logits, dim=1),
                                                  F.softmax(teacher_logits_org.detach(), dim=1))
                    ...
                    loss = (kl_loss1+kl_loss2) / 2  #(kl_loss1 + kl_loss2) / 2
                    ...
                    pbar.set_postfix(**{'Train Loss': np.mean(losses),
                                        'Train Loss k1': kl_loss1.item(),
                                        'Train Loss k2': kl_loss2.item(),
