...
class MaeFinetuneTrainer(BaseTrainer):
    ...
    def train(self):
        ...
        for e in range(self.total_epoch):
            ...
            apgd = APGDAttack(self.model, n_restarts=1, n_iter=20, verbose=False,
            ...
            with tqdm(total=train_step, desc=f'Train Epoch {e + 1}/{self.total_epoch}', postfix=dict,
                ...
                for img, label in iter(self.train_dataloader):
                    ...
                    with self.accelerator.autocast():
                        ...
                        adv_img_teacher = fgsm_attack(self.teacher_model, img, label, self.loss_fn, epsilon=12 / 255)
                        with torch.no_grad():
                            self.teacher_model.eval()
                        ...
                        loss = distill_loss(logits, teacher_logits, tau=5) + distill_loss(logits_org, teacher_logits, tau=1)  #+trade_loss/5  #+ self.loss_fn(logits,label)  #+distill_loss(logits_org,teacher_logits_org)  #+self.loss_fn(self.model(img), label)
                    ...
                    #
                    #     move_temp = adv_img
                    #     temp_acc = student_acc
                    #     move_temp_acc = student_acc
                    # else:
                    #     temp_logits = self.model(temp)
                    #     temp_acc = acc_fn(temp_logits, temp_label)
                    #
                    #     move_temp = trades_loss_with_adv(self.model, temp_img, temp_label, step_size=epsilon / perturb_steps/4,
                    #                                      epsilon=epsilon, perturb_steps=1,
                    #                                      optimizer=self.optim, beta=8, x_adv=move_temp)
                    #
                    #     # trade_loss, move_temp = trades_loss(self.model, temp_img, temp_label, step_size=epsilon / perturb_steps,
                    #     #                                   epsilon=epsilon, perturb_steps=perturb_steps,
                    #     #                                   optimizer=self.optim, beta=8)
                    #
                    #
                    #     move_temp_logits = self.model(move_temp)
                    #     move_temp_acc = acc_fn(move_temp_logits, temp_label)
                    ...
                    # move_temps.append(move_temp_acc.item())
                    ...
                    pbar.set_postfix(**{'Train Loss': np.mean(losses),
                                        ...
                                        # 'move_temp_acc': np.mean(move_temp_acc.item()),
