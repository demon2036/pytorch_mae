...
class MaeFinetuneTrainer(BaseTrainer):
    def __init__(self,
        ...
        super().__init__(seed, batch_size, max_device_batch_size, total_epoch, mixed_precision, save_every=save_every,
                         transform=True, use_aux_dataset=use_aux_dataset, unsup_fraction=unsup_fraction,
                         aux_data_filename=aux_data_filename)

        self.loss_fn = loss_fn
        self.model = model_instant_function(model_target, pretrained_model_path, is_mae=True).to('cuda')
        self.optim = torch.optim.AdamW(self.model.parameters(),
                                       lr=base_learning_rate * batch_size / 256,
                                       betas=(0.9, 0.999), weight_decay=weight_decay
                                       )

        # self.optim = timm.optim.create_optimizer_v2(self.model, opt='adanw', lr=1.0e-3, weight_decay=0.05)
        ...
        self.teacher_model = load_model(model_name='Cui2023Decoupled_WRN-28-10', dataset='cifar10',
            ...
            with tqdm(total=train_step, desc=f'Train Epoch {e + 1}/{self.total_epoch}', postfix=dict,
                ...
                for img, label in iter(self.train_dataloader):
                    ...
                    with torch.cuda.amp.autocast(enabled=False):
                        ...
                        # logits_adv, logits_distill_adv = self.model.forward_train(adv_img)
                        logits_adv = self.model(img)

                        # teacher_adv = fgsm_attack(self.teacher_model, img, label, self.loss_fn,epsilon=max((40-e)/255,8/255) )
                        with torch.no_grad():
                            self.teacher_model.eval()
                            teacher_logits_adv = self.teacher_model(adv_img)
                    ...
                    # loss = distill_loss(logits_adv,teacher_logits_adv) +distill_loss(logits,teacher_logits) #+ self.loss_fn(logits,label)  #+distill_loss(logits_org,teacher_logits_org)  #+self.loss_fn(self.model(img), label)
                    # loss = distill_loss(logits, teacher_logits)
                    loss_adv = distill_loss(logits_adv,
                                            teacher_logits_adv)  #+ distill_loss(logits_distill_adv, teacher_logits_adv)

                    # loss_distill = distill_loss(logits_distill_adv, teacher_logits_adv)
                    # loss = (loss_distill + loss_adv) / 2
                    loss = loss_adv



                    teacher_acc = acc_fn(teacher_logits_adv, label)
                    ...
                    with torch.cuda.amp.autocast():
                        img = img.to('cuda').detach()

                        label = label.to('cuda').detach()

                        step_count += 1
                        adv_img_shadow = fgsm_attack(self.shadow_model, img, label, self.loss_fn, )
                        logits_shadow = self.shadow_model(adv_img)

                        # teacher_adv = fgsm_attack(self.teacher_model, img, label, self.loss_fn,epsilon=max((40-e)/255,8/255) )
                        with torch.no_grad():
                            teacher_logits_shadow = self.teacher_model(adv_img_shadow)
                    loss_shadow = distill_loss(logits_shadow, teacher_logits_shadow)

                    loss_shadow.backward()

                    for p, shadow_p in zip(self.model[1].parameters(), self.shadow_model[1].parameters()):#zip(self.model[1].head.parameters(), self.shadow_model[1].head.parameters()):
                        print(p.grad.norm(), shadow_p.grad.norm())
                        print()
                    ...
                    pbar.set_postfix(**{'Train Loss': np.mean(losses),
                                        ...
                                        # 'logits_distill_adv': loss_distill.item(),
