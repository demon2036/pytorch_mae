...
# entropy = -(output * torch.log2(output + 1e-7) + one_minus_output * torch.log2(one_minus_output + 1e-7)).mean()


class Router(nn.Module):
    def __init__(self, dim, num_experts):
        super().__init__()
        self.gate = nn.Linear(dim, num_experts, dtype=torch.float32)

    def forward(self, x):
        # with torch.cuda.amp.autocast(enabled=False):
        #     y = self.gate(x)
        # with autocast(dtype=torch.float32, enabled=False):
        #     y = self.gate(x.to(torch.float32))
        y = self.gate(x)
        # y=F.softmax(y,1,dtype=torch.float)
        # y = self.gate(x.to(torch.float32))
        # print(y.dtype)
        return y
...
class MoeLayer(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, train=False):
        inputs_shape = inputs.shape
        inputs = inputs.view(-1, inputs_shape[-1])

        gate_logits = self.gate(inputs)

        # print(gate_logits.dtype)
...
class BlockMoE(nn.Module):
    def __init__(
        ...
        self.mlp = MoeLayer(experts=[mlp_layer(
            ...
            act_layer=act_layer,
            drop=proj_drop,
        ) for _ in range(num_experts)], gate=Router(dim, num_experts), )
    ...
    def forward(self, x: torch.Tensor, train=False) -> torch.Tensor:
        x = x + self.drop_path1(self.ls1(self.attn(self.norm1(x))))
        x = x + self.drop_path2(self.ls2(self.mlp(self.norm2(x), train=train)))
        return x
