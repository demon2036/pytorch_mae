...
def pgd_loss(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0,
                attack='linf-pgd', label_smoothing=0.1, use_cutmix=False, ):
    """
    TRADES training (Zhang et al, 2019).
    """
    ...
    if attack == 'linf-pgd':
        for _ in range(perturb_steps):
            x_adv.requires_grad_()
            with torch.enable_grad():
                loss_kl = criterion_indiv(model(x_adv), p_natural)
            grad = torch.autograd.grad(loss_kl, [x_adv])[0]
            x_adv = x_adv.detach() + step_size * torch.sign(grad.detach())
            x_adv = torch.min(torch.max(x_adv, x_natural - epsilon), x_natural + epsilon)
            x_adv = torch.clamp(x_adv, 0.0, 1.0)
...
def trades_loss_with_adv(model, x_natural, y, optimizer, step_size=0.003, epsilon=0.031, perturb_steps=10, beta=1.0,
                         attack='linf-pgd', label_smoothing=0.1, use_cutmix=False, x_adv=None):
    """
    TRADES training (Zhang et al, 2019).
    """
    ...
    if x_adv is None:
        ...
        # delta=(x_natural-x_adv)
        # delta=torch.clamp(delta,-perturb_steps*step_size,perturb_steps*step_size)
